start(){
docker start sw0 sw1 sw2 sw3 sw4 sw5 sw6 sw7 sw8 sw9 sw10 sw11 sw12 sw13 sw14 sw15 sw16 sw17 sw18 sw19 sw20 sw21 sw22 sw23 sw24 sw25 sw26 sw27 sw28 sw29 sw30 sw31 h0 h1 h2 h3
}
run(){
...
#sw3 x h0
docker exec sw3 ovs-vsctl add-port s3 gre5 -- set interface gre5 type=gre options:remote_ip=172.28.5.32
docker exec h0 ovs-vsctl add-port h0 gre0 -- set interface gre0 type=gre options:remote_ip=172.28.5.4

#sw2 x h1
docker exec sw2 ovs-vsctl add-port s2 gre5 -- set interface gre5 type=gre options:remote_ip=172.28.5.33
docker exec h1 ovs-vsctl add-port h1 gre0 -- set interface gre0 type=gre options:remote_ip=172.28.5.3

#sw5 x h2
docker exec sw5 ovs-vsctl add-port s5 gre5 -- set interface gre5 type=gre options:remote_ip=172.28.5.34
docker exec h2 ovs-vsctl add-port h2 gre0 -- set interface gre0 type=gre options:remote_ip=172.28.5.6

#sw6 x h3
docker exec sw6 ovs-vsctl add-port s6 gre5 -- set interface gre5 type=gre options:remote_ip=172.28.5.35
